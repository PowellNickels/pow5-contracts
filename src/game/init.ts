...
async function initializeGame(
...

  {
    ...
    const pow1Token0: string = (
      await pow1MarketPoolContract.token0()
    ).toLowerCase();
    const pow1Token1: string = (
      await pow1MarketPoolContract.token1()
    ).toLowerCase();
    if (
    ...
    const pow5Token0: string = (
      await pow5StablePoolContract.token0()
    ).toLowerCase();
    const pow5Token1: string = (
      await pow5StablePoolContract.token1()
    ).toLowerCase();
    if (
    ...
    const pow1SqrtPriceX96 = (await pow1MarketPoolContract.slot0())
      .sqrtPriceX96;
    const pow5SqrtPriceX96 = (await pow5StablePoolContract.slot0())
      .sqrtPriceX96;
    ...
    if (pow1SqrtPriceX96 !== 0n) {
      ...
      transactions.push(
        pow1MarketPoolContract.initialize(
          encodePriceSqrt(
    ...
    if (pow5SqrtPriceX96 !== 0n) {
      ...
      // Initialize the Uniswap V3 pool
      transactions.push(
        pow5StablePoolContract.initialize(
          encodePriceSqrt(
