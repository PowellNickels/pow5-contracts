...
contract DeFiManager is ReentrancyGuard, AccessControl, IDeFiManager {
  ...
  /**
   * @dev See {IDeFiManager-borrower}
   */
  function borrower(uint256 tokenId) external view override returns (address) {
    // Read state
    return _tokenIdToBorrower[tokenId];
  }

  /**
   * @dev See {IDeFiManager-setBorrower}
   */
  function setBorrower(
    ...
    // Update state
    _tokenIdToBorrower[tokenId] = borrower_;
  }
