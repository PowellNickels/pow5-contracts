  ...
  DutchAuctionBase
{
  ...
  function purchase(
    ...
    // Handle the tip
    {
      ...
      // Update last sale price in auction metadata
      _auctionMetadata.lastSalePriceBips = currentPriceBips;

      // Calculate the auction tip amounts
      uint256 pow1TipAmount = uint256(
        wadMul(int256(pow1Amount), int256(currentPriceBips))
      );
      uint256 marketTipAmount = uint256(
        wadMul(int256(marketTokenAmount), int256(currentPriceBips))
      );
